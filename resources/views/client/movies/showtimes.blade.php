                ...
                <div class="showtimes-panel">
                    ...
                    <div class="date-selector" id="date-selector">
                        @foreach ($availableDates as $date)
                            <div class="date-item {{ $date['date'] == $selectedDate ? 'active' : '' }}"
                                data-date="{{ $date['date'] }}" onclick="changeDate('{{ $date['date'] }}')">
                        ...
                        <select class="cinema-select" id="cinema-select" onchange="changeCinema(this.value)">
                            ...
                            @foreach ($availableCinemas as $cinema)
                                <option value="{{ $cinema->cinema_id }}"
                                    {{ $selectedCinema == $cinema->cinema_id ? 'selected' : '' }}>
                    ...
                    <div class="error-state" id="error-state" style="display: none;">
                        <svg class="error-icon" fill="currentColor" viewBox="0 0 20 20">
                            <path fill-rule="evenodd"
                                d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7 4a1 1 0 11-2 0 1 1 0 012 0zm-2-9a1 1 0 00-1 1v4a1 1 0 102 0V6a1 1 0 00-1-1z"
                                clip-rule="evenodd"></path>
                        </svg>
                ...
                <div class="tab-panel" id="panel-trailer">
                    <div class="showtimes-panel">
                        ...
                        <div class="aspect-video rounded-2xl overflow-hidden shadow-lg">
                            <iframe src="{{ $movie->trailer_url }}" title="Trailer {{ $movie->title }}"
                                class="w-full h-full"
            ...
            <div class="tab-panel" id="panel-reviews">
                <div class="showtimes-panel">
                    ...
                    <div id="reviews-root" class="py-2">
                        <div class="text-center py-12" id="reviews-loading-placeholder">
                            <div class="spinner"
                                style="width:36px;height:36px;border-width:3px;border-top-width:3px;margin:0 auto 1rem">
                            </div>
                            <div class="text-gray-600">Đang tải đánh giá...</div>
                        </div>
                    </div>
                </div>
            </div>
...
@push('scripts')
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Preload reviews so content is ready when user opens the tab
            loadReviews();
        });
    ...
    <script>
        function loadReviews() {
            ...
            reviewsRoot.innerHTML = `
                <div class="text-center py-12" id="reviews-loading-placeholder">
                    <div class="spinner" style="width:36px;height:36px;border-width:3px;border-top-width:3px;margin:0 auto 1rem"></div>
                    <div class="text-gray-600">Đang tải đánh giá...</div>
                </div>
            `;
            fetch(`/movies/{{ $movie->movie_id }}/reviews`, {
                ...
                .catch(error => {
                    ...
                    reviewsRoot.innerHTML =
                        `<div class="text-center text-red-500">Failed to load reviews: ${error.message}</div>`;
                });
        }
        ...
        document.getElementById('reviews-root').addEventListener('submit', function(e) {
            ...
            } // <--- 2. Đóng IF (Bạn bị thiếu dấu này)

        }); // <--- 3. Đóng Event Listener
    </script>
@endpush
