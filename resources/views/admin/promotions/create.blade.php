                            ...
                            <div class="col-md-6">
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="form-control @error('discount_type') is-invalid @enderror" 
                                        ...
                                        <option value="Fixed Amount" {{ old('discount_type') == 'Fixed Amount' ? 'selected' : '' }}>Số tiền cố định (VNĐ)</option>
                                    </select>
                                    @error('discount_type')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
...
$(document).ready(function() {
    ...
    if (!$('#start_date').val()) {
        $('#start_date').val(tomorrow.toISOString().slice(0, 16));
    }
    if (!$('#end_date').val()) {
    ...
    $('#promotionForm').submit(function(e) {
        ...
        if (startDate <= new Date()) {
            if (!confirm('Ngày bắt đầu đã qua. Bạn có muốn tiếp tục?')) {
                e.preventDefault();
                return false;
            }
        }
